...
do
    ...
    python3 create_data_cube.py --boxsize 25 --npix 128 --npix_z 128 --threshold $threshold --catalog_threshold 0.1 --gpu_id $gpu_id --gen_both --prob_threshold 1e-5 --input_fname $input_fname --output_catalog_fname $output_dir/group.catalog_data.${snapshot_number}.threshold${threshold}.${model_name}.seed${seed}.h5 --output_fname $output_dir/group.data_cube.${snapshot_number}.threshold${threshold}.${model_name}.seed${seed}.h5 --model_dir $model_dir --seed $seed --global_param_file $global_param_file --global_param_id $global_param_id
