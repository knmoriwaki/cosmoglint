...
def load_halo_data(data_dir, max_length=10, use_numpy=False, use_rad=False, ndata=None):
    
    norm_params = np.loadtxt("./norm_params.txt")
    xmin, xmax = norm_params[0]
    ymin, ymax = norm_params[1]
    ymin_rad, ymax_rad = norm_params[2]

    x_list = []
    y_list = []

    count = 0
    for snapshot_number in [33]:
        file_path = os.path.join(data_dir, f"group.central-satellite.{snapshot_number}.txt")
        file_path_rad = os.path.join(data_dir, f"group.central-satellite.rad.{snapshot_number}.txt")
        with open(file_path, 'r') as f, open(file_path_rad, "r") as f_rad:
            for line, line_rad in zip(f, f_rad):
                line = line.strip()
                line_rad = line_rad.strip()

                if not line or line.startswith("#"):
                    continue
                
                tokens = line.split()
                tokens_rad = line_rad.split()
                ...
                if np.abs( float(tokens_rad[0]) - m_val ) > 1e-4:
                    raise ValueError("Mass values are not consistent!")
                
                ### central luminosity
                try:
                    l_central = float(tokens[1])
                    rad_central = float(tokens_rad[1])
                    if l_central < ymin:
                        l_central = ymin
                except ValueError:
                    l_central = ymin
                    rad_central = ymin_rad

                ### satellite luminosities
                try:
                    l_values = [float(tok) for tok in tokens[2:]]
                    rad_values = [float(tok) for tok in tokens_rad[2:]]
                except ValueError:
                    l_values = []
                    rad_values = []

                sorted_indices = sorted(range(len(l_values)), key=lambda k: l_values[k], reverse=True)
                l_values = [l_central] + [l_values[i] for i in sorted_indices]
                rad_values = [rad_central] + [rad_values[i] for i in sorted_indices]

                l_values = l_values[:max_length]
                rad_values = rad_values[:max_length]

                rad_values = [ r if r > 10**ymin_rad else 10**ymin_rad for r in rad_values ]
                rad_values = [ np.log10(r) for r in rad_values ]

                ### normalization
                m_val = (m_val - xmin) / (xmax - xmin)
                l_values = (l_values - ymin) / (ymax - ymin)
                rad_values = (rad_values - ymin_rad) / (ymax_rad - ymin_rad)

                ### append
                x_list.append(m_val)

                if use_rad:
                    y = np.array([l_values, rad_values], dtype=np.float32) # (2, max_length)
                else:
                    y = np.array([l_values], dtype=np.float32) # (1, max_length)
...
class MyDataset(Dataset):
    def __init__(self, data_dir, max_length=10, use_rad=False, ndata=None):
        
        self.x, self.y = load_halo_data(data_dir, max_length=max_length, use_rad=use_rad, ndata=ndata)
